...
task main()
{
	...
	while (true)
	{
	  ...
	  while (true) // wait for button press
	  {
		...
		//increments the number of button presses every time a button is pressed
	  switch (nBtn)
	  {
		...
		//displays the current number of button presses
		playSoundFile("! Click.rso");
	  ++nCycles;
	  displayTextLine(0, "  Cycles: %d", nCycles);
	  displayTextLine(1, "");
	  displayTextLine(2, "Left Enter Right");
	  stringFormat(sTemp,   "%4d %5d", nLeftButton, nEnterButton);
	  displayTextLine(3, "%s %5d", sTemp, nRightButton);
	  displayTextLine(4, "  None    Exit  ");
	  displayTextLine(5, " %5d    %5d", nNoButton, nExitButton);
	  displayTextLine(6, "TripleClick EXIT");
	  displayTextLine(7, " to end program ");
