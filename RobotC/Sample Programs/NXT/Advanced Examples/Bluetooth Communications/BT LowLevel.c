...
task main()
{
  ...
  checkBTLinkConnected();
	...
	while (true)
	{
		...
		//
		displayBigStringAt(0, 31, "Xmit Rcv");
	  displayBigStringAt(0, 15, "  %02X  %02X", nMsgXmit, nLastCharReceived & 0x00FF);
...
task sendRawData()
{
	...
	while (true)
	{
		...
		++nMsgXmit;
		nMsgXmit %= 256;
		memset(BytesToSend, nMsgXmit, sizeof(BytesToSend));
